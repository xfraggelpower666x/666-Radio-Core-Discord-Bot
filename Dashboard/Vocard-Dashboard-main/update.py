...
GITHUB_API_URL = "https://api.github.com/repos/ChocoMeow/Vocard-Dashboard/releases/latest"
VOCARD_DASHBOARD_URL = "https://github.com/ChocoMeow/Vocard-Dashboard/archive/"
...
def check_version(with_msg=False):
    ...
    if with_msg:
        msg = f"{bcolors.OKGREEN}Your dashboard is up-to-date! - {latest_version}{bcolors.ENDC}" if latest_version == __version__ else \
              f"{bcolors.WARNING}Your dashboard is not up-to-date! The latest version is {latest_version} and you are currently running version {__version__}\n. Run `python update.py -l` to update your dashboard!{bcolors.ENDC}"
        print(msg)
...
def download_file(version=None):
    ...
    print(f"Downloading Vocard Dashboard version: {version}")
    response = requests.get(VOCARD_DASHBOARD_URL + version + ".zip")
...
def install(response, version):
    ...
    if user_input.lower() in ["y", "yes"]:
        ...
        source_dir = os.path.join(ROOT_DIR, f"Dashboard-{version}")
        if os.path.exists(source_dir):
            for filename in os.listdir(ROOT_DIR):
                if filename in IGNORE_FILES + [f"Dashboard-{version}"]:
                    continue
            ...
            os.rmdir(source_dir)
        print(f"{bcolors.OKGREEN}Version {version} installed Successfully! Run `python main.py` to start your dashboard{bcolors.ENDC}")
    else:
        print("Update canceled!")

def parse_args():
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(description='Update script for Vocard Dashboard.')
    parser.add_argument('-c', '--check', action='store_true', help='Check the current version of the Vocard Dashboard')
    parser.add_argument('-v', '--version', type=str, help='Install the specified version of the Vocard Dashboard')
    parser.add_argument('-l', '--latest', action='store_true', help='Install the latest version of the Vocard Dashboard from Github')
    parser.add_argument('-b', '--beta', action='store_true', help='Install the beta version of the Vocard Dashboard from Github')
